def prompt_to_sql(prompt):
    ...
    # --- Maximum alerts by district/area ---
    if ("district" in prompt or "area" in prompt) and "maximum" in prompt:
        return (
            "SELECT area_description, COUNT(*) AS total_alerts FROM alerts GROUP BY area_description ORDER BY total_alerts DESC LIMIT 1;",
            "District with the highest number of alerts"
        )

    # --- Top 5 districts or areas ---
    if ("top" in prompt and ("district" in prompt or "area" in prompt)) or "top 5 districts" in prompt or "top five districts" in prompt:
        return (
